...
| 2026-09-13 | **侧车「手动跑像卡住」给出自解释**（弈韬真机跑 exe 后问「输出是这些内容，是什么情况」）。看到的只有 `runtime.ready` + 每 5 秒一条 `runtime.heartbeat`，然后没有下文 —— 这**是正常的**：它是 stdio 侧车，等的是父进程管道里的帧，人在终端里跑就永远等不到；但对不熟悉这套协议的人，「进程不出声」和「进程挂了」长得一模一样。新增 `runtime.console_hint()`：`stdin` 是终端时往 **stderr** 写一段说明（含两条可执行的出路 `--selfcheck` / `scripts/test_sidecar.py`），管道场景一个字都不写（Tauri 走管道，那儿多写一行纯属噪音）。守卫 `worker/tests/test_console_hint.py` 4 条，两条红线：① 只在终端出现 —— 顺带容错**没有 `isatty` 的替身**（`test_main_concurrency.py` 用 `SimpleNamespace(buffer=…)` 伪装 stdin，探针崩掉会把整条启动路径带下去）与 `isatty` 抛 `ValueError` 的已关闭流；② 提示**绝不落进 stdout** —— stdout 是帧通道，帧是**长度前缀**的，混进任意非帧字节会让父进程解析器读到半截且**无法自愈**，所以判据写成「按帧协议把 stdout 解回来能否完整解通」而不是「文本里不含某个词」。三组负向验证**各自单独**命中并点名到具体行：提示改写 stdout → 点名第 140 行帧解析（泄漏字节正是提示文案）；去掉 `AttributeError` 容错 → 点名第 99 行；去掉 `ValueError` 容错 → 点名第 101 行。`worker/README.md` 增「手动跑起来『像卡住』是正常的」小节。<br>**真机复核时又抓到一个假阳性**：Windows 的 `NUL` 是**字符设备**，CRT 的 `_isatty` 只问「是不是字符设备」→ `isatty()` 对它返回 `True`（实测 3.12.4：`subprocess.DEVNULL` → `True`、`subprocess.PIPE` → `False`、`open("NUL")` → `True`）。于是 `< NUL` / `subprocess.DEVNULL` / Rust `Stdio::null()` 全被当成「人在终端前」，给管道式父进程刷没用的 stderr。**主链路本来就对**（Tauri 走 pipe），错的是假阳性 → 加 `_is_console_handle()` 用 `GetConsoleMode` 收窄（只对真控制台句柄成功；非 Windows 与拿不到 `fileno` 的替身放行，且**拆成独立函数**才保住替身测试的可测性）。两轮新探针 D/E 各自单独点名（D → NUL 测试第 114 行；E → 替身测试第 84 行）。真机端 `.workbuddy/packaging-acceptance/probe_console_hint.py`：把**真控制台句柄**（`CONIN$`）接到 stdin → stderr 700B（GBK）提示如预期出现、stdout 476B 恰是 `ready` + 2 心跳且全部按帧解通；`< /dev/null` → stderr 只剩日志、stdout 234B 一条 ready。**同一轮还量出一个被指标掩盖的事实**：冻结 onefile 的**首字节实测 +4.97 s**（源码模式 0.7 s），而 `startup_ms` 是从 Python 起算的 —— **它不含 bootloader 解包 70 MB 的时间**，所以那个「140 ms」不是用户感知的启动延迟 |
| 2026-09-14 | **桌面端完整打包首次打通**（`npx tauri build` → NSIS）：产出 `STEPWORK_0.1.0_x64-setup.exe` **73.3 MB**。此前所有工作停在「sidecar 有了」，`src-tauri/target/` **根本不存在**（Rust 一次没编译过）——这次是第一次冷编译。<br>三件「不看底盘正的日常错觉」：<br>① **CLI 不用另装** —— `~/.cargo/bin` 里确实没有 `cargo-tauri`，但 `@tauri-apps/cli` 在 `apps/desktop/node_modules` 里，直接 `npx tauri build` 就行（差点因为「CLI 没装」而放弃）。<br>② **依赖不用下载** —— `Cargo.lock` 488 个 package，本地 `.cargo/registry` 已缓存 481 个，冷编译是纯 CPU 开销（9 分 48 秒）。<br>③ **判「资源有没有打进安装包」不能靠体积推断、也不能靠字符串搜** —— NSIS 是 `LZMA:23` **实心压缩**，搜 `stepwork-worker.exe` 零命中（会误判成没打进去），而 7-Zip 列表里它的 `Size` 列是**空的**。真正的证据是 `7z l -slt` 逐条读元数据：`Modified=2026-09-14 19:49:44` 与 `dist/` 原件**同秒**。同时确认 `$TEMP/MicrosoftEdgeWebview2Setup.exe`（1.7 MB）已按 `webviewInstallMode=embedBootstrapper` 打进去 → 离线装机能起 UI。<br>**仍未做**：装到本机实跑。安装包出了 ≠ 能用 —— 安装路径下的 `STEPWORK_HOME` 落点、父进程拉起侧车后的真链路、UI 是否起得来都没验。侧车的单体验收（`accept_sidecar.py` + `probe_console_hint.py`）与整机联调是两回事。<br>顺带：同一份源码两次打包**字节数不同**（72,979,064 vs 72,978,219，差 845 B）——单文件 exe 里的归档元数据含时间戳，**别拿 exe 体积当「内容是否一致」的判据** |
| 2026-09-17 | **系统化代码评审十三轮收口**（`yt-dev-review` 三维九域，Python + Rust 对称 + 覆盖率盲区）：P0×7 + P1×19 + P2×16 逐项修复并配回归锁。要点 —— Restore 漏重绑 2/7 子 repo、日志掩码两套清单泄漏 `passphrase/credential/key` 且碎 JSON、终态复活竞态、async 里 subprocess 无超时冻结事件循环、每命令重载 Whisper 模型、共享 httpx client 与代理 TTL、MCP stderr 互锁、逐幕并发、project_io 事务/zip 形状/炸弹、幂等 `reserve/release` 原子化、迁移 0015 修跨 workspace 审计泄露、`DispatchError`/`validation`/`build_deps`/Provider 表驱动去重、Rust stderr 多字节 drain panic 与 watchdog restart storm。Python 997→**1106 tests**、Rust 2→**19**；`ruff`/`mypy strict 244`/`cargo clippy -D warnings` 全绿。commit `47c825c..ff31612`。**详录与"刻意不做"的性价比裁决见 [`reviews/2026-09-17-yt-dev-review-13-rounds.md`](./reviews/2026-09-17-yt-dev-review-13-rounds.md)** |
